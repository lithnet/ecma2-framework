...
namespace Lithnet.Ecma2Framework.Internal
{
    ...
    /// </summary>
    internal class Ecma2Initializer
    {
        ...
        private void AddInternalServices(IConfiguration configuration)
        {
            ...
            // The engine-services accessor (Path C). A single mutable singleton, registered as both its concrete
            // type and the IEngineServices interface so the worker can resolve the instance to set the host's
            // injected ExtensionsDirectory on connection open, while providers resolve the interface to read it.
            EngineServices engineServices = new EngineServices();
            this.Services.AddSingleton(engineServices);
            this.Services.AddSingleton<IEngineServices>(engineServices);
        }
    }
}
